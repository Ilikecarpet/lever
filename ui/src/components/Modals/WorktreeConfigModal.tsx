...
export default function WorktreeConfigModal({ open, worktreeId, onClose }: Props) {
  const worktree = useWorktreeStore((s) => s.worktrees.find((w) => w.id === worktreeId));
  ...
  const [editing, setEditing] = useState<{ groupId: string; serviceId: string } | null>(null);
  ...
  const openAddForm = () => {
    ...
    if (!groupId) {
      ...
      const gid = `group-${n}`;
      addWorktreeGroup(worktreeId, { id: gid, label: `Group ${n}`, services: [] });
      saveConfig();
      groupId = gid;
    }
  ...
  const handleSave = () => {
    ...
    if (editing) {
      ...
      if (editing.groupId === targetGroupId) {
        updateWorktreeService(worktreeId, editing.groupId, editing.serviceId, svc);
      } else {
        updateWorktreeService(worktreeId, editing.groupId, editing.serviceId, svc);
        moveWorktreeService(worktreeId, editing.serviceId, editing.groupId, targetGroupId);
      }
    } else {
      ...
      addWorktreeService(worktreeId, targetGroupId, svc);
    }
  ...
  const handleRenameConfirm = (groupId: string, value: string) => {
    ...
    if (name) {
      updateWorktreeGroup(worktreeId, groupId, { label: name });
      saveConfig();
    }
      ...
      >
        <div className={styles.modal}>
          <div className={styles.modalHeader}>
            <h2>Manage Services — {worktree.branch}</h2>
          ...
          <div className={styles.modalBody}>
            {groups.length === 0 && (
              <div className={styles.emptyGroup} style={{ padding: 24 }}>
                No service groups yet. Click "+ Add Service" to get started.
              </div>
            )}
            {groups.map((group) => (
                  ...
                  group.services.map((svc) => (
                    <div key={svc.id} className={styles.si}>
                      ...
                      <div className={styles.siActions}>
                        ...
                        <button
                          className={`${styles.mBtn} ${styles.mBtnSm} ${styles.mBtnDanger}`}
                          onClick={() => handleDeleteService(group.id, svc.id)}
                        >
                          Delete
                        </button>
                      </div>
